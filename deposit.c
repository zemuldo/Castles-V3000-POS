...
void transactdeposit(void) {
    ...
    iBatchNo = 0;
        ...
        deposit_StartTrans(d_INIT_TRANS);
...
void depositPrint_Receipt(EMVCL_RC_DATA_EX *data, BYTE isNeedSignature, ULONG ulValue) {
    //------------------------------------------------------
    //PrintBlank();
    ...
    //Trans Type
    CTOS_PrinterPutString("Trans Type : Card Deposit");
    CTOS_PrinterPutString(" ");
...
void deposit_StartTrans(BYTE bType) {
    BYTE bKey;
    BYTE NeedSignature;
    BYTE temp[20];
    BYTE baAmount[32];
...
ex:
  ...
  //=================================================================================================================//
    ...
    if (usTxResult == d_EMVCL_OUTCOME_ONL) {
       ...
       deposit_post(pin, g_baInputAmt, cardvendor, cardnumber, expdate,mpin,mcardvendor, mcardnumber, mexpdate);
    ...
    depositPrint_Receipt(&stRCDataEx, NeedSignature, g_ulAmt);
    if (NeedSignature) {
